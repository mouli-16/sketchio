...
**D0 not forget to import the css files**
...
U can make a separate folder in your react app only say ```service``` and then create another file in this folder say ```socket.js```
 
- Install ```socket.io-client``` a built in library in react that helps us with socket connection
```npm i socket.io-client```
 
- import io from "socket.io-client";
 
- Create an endpoint and decide the port where it will run
```export const socket = io(ENDPOINT);```
 
 
- Go to ```Board.jsx``` and import ```{socket}``` from ```socket.js```
- Go to the onPaint function and add ```socket.emit``` function that emits the canvas data from the base 64 encoded image string
```socket.emit("canvas-data", base64ImageData);```
 
- Now ```”canvas-data”``` event has to be same with the event that receives the data
- Go to the Board class constructor ```socket.on```and write a function that uses data which is received by the server in the form of an image source which is the base 64 encoded image string.
